...
/* 
Consider a non-prime number n and its factors a and b where a * b = n. If both a and b are greater than the square root of n, then their product would be greater than n. Therefore, at least one of a or b must be less than or equal to the square root of n. By checking for divisors only up to the square root of n, you effectively cover all possible divisors without redundant iterations.

For example, if n is not a prime number and has a divisor greater than its square root, then there must be another divisor smaller than its square root. Therefore, it's not necessary to check for divisors beyond the square root.

This optimization significantly improves the efficiency of the algorithm, especially for larger numbers, by reducing the number of iterations needed in the loop. It's a common technique used in prime-checking algorithms.
*/
...
/* 
The problem requires generating all possible combinations of well-formed
parentheses of length 2n. To solve this, we can use a recursive approach.
At each step, we have two choices: either add an opening parenthesis "("
or add a closing parenthesis ")". However, we need to make sure that the
number of opening parentheses is always greater than or equal to the
number of closing parentheses, so that the parentheses remain well-formed.
*/
...
result: a reference to the array of strings where we store the generated combinations.
...
In the generateParentheses function, we first check if the length of the current string is equal to 2n. If it is, we have generated a valid combination, so we add it to the result arrau and return.

- If the length of current is not equal to 2n, we have two choices:

A) If the count of opening parentheses open is less than n, we can add an opening parenthesis to the current combination and make a recursive call to generateParentheses, incrementing the open count by 1.
B) If the count of closing parentheses close is less than the open count, we can add a closing parenthesis to the current combination and make a recursive call to generateParentheses, incrementing the close count by 1.

- In the generateParenthesis function, we initialize an empty result array and call the generateParentheses function with the initial values of current as an empty string, open and close counts as 0, and n as the input value.

- Finally, we return the result array containing all the generated combinations of well-formed parentheses.
*/
